...
import { registerMealIngredients } from "@/actions/food-action";
...
const DialogIngredient = ({ ingredients, mealId }: IngredientsDialogProps) => {
  ...
  const [arrQuantityUnit, setArrQuantityUnit] = useState<
    QuantityUnitStructure[]
  >([]);
  ...
  const updateQuantityUnit = (idIngredient: string) => {
    ...
    if (ingredient) {
      setArrQuantityUnit(
        ingredient.ingredientCalories.map(
          (ingredientCalories) => ingredientCalories.quantityUnit
        )
      );
    }
  };
  ...
  const onSubmit = async (data: z.infer<typeof MealIngredientsSchema>) => {
    startTransition(() => {
      registerMealIngredients(data,mealId).then((data) => {
        console.log(data);
        form.reset();
        router.refresh();
  ...
  return (
    <Dialog>
      <DialogTrigger asChild>
        <div>
          <Button className="w-1/6 hidden lg:block">Add ingredient</Button>
          <Button className="w-1/3 lg:hidden">Add ingredient</Button>
        </div>
      </DialogTrigger>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Add your ingredient for the meal{" "}
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form
                        ...
                        >
                          {field.value
                            ? arrQuantityUnit.find(
                                (quantityUnit) =>
                                  quantityUnit.idQuantityUnit === field.value
                              )?.name
